...
const copy = {
  en: {
    ...
    origin: {
      ...
      highlight:
        "Strong origin performs better when supported by product, positioning, and presentation.",
  ...
  ar: {
    ...
    origin: {
      ...
      highlight:
        "الأصل القوي يحقق أداءً أفضل عندما يكون مدعوماً بالمنتج والتموضع والتقديم الفاخر.",
      whatsappCta: "الاتصال عبر الواتساب",
    },
  },
} as const;

const framerEase: [number, number, number, number] = [0.22, 1, 0.36, 1];
...
export default function WhyBrazil({ locale }: WhyBrazilProps) {
  ...
  const whatsappUrl = "https://wa.me/5535999430852";
    ...
    <>
      ...
      >
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 z-10 bg-gradient-to-b from-[#F5F5F3] via-[#F5F5F3]/40 to-[#3e5344]/80" />
        </div>
        ...
        <motion.div
          ...
          className="relative z-20 mx-auto flex w-full max-w-[1240px] flex-col gap-32 px-6 lg:px-8"
        >
          <div className="grid grid-cols-1 gap-10 xl:grid-cols-12">
            <motion.div
              variants={fadeUp}
              className="flex flex-col justify-center xl:col-span-8"
            >
              <div className="flex flex-col gap-6">
                ...
                <h2 className="mb-4 text-balance text-4xl font-normal uppercase leading-[1.05] tracking-tight text-[#111] sm:text-5xl lg:text-6xl">
                  {text.brand.headline}
                </h2>
                <p className="max-w-[700px] text-lg font-light leading-relaxed tracking-wide text-[#333] md:text-xl">
                  {text.brand.text}
                </p>
              </div>
            </motion.div>
            ...
            >
              {text.brand.points.map((point, index) => (
                <div
                  key={index}
                  className="group relative flex items-center gap-5 rounded-2xl border border-black/5 bg-white/30 p-6 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:bg-white/60 hover:shadow-xl"
                >
          ...
          <motion.div
            variants={fadeUp}
            className="relative overflow-hidden rounded-[2.5rem] border border-white/40 bg-white/15 p-8 shadow-2xl backdrop-blur-[40px] md:p-16 lg:p-20"
          >
            <div className="relative z-10 flex h-full flex-col gap-16 lg:gap-20">
              <div className="flex w-full flex-col gap-6">
                ...
                <p className="w-full text-pretty text-lg font-light leading-relaxed tracking-wide text-black md:text-xl">
                  {text.origin.text}
                </p>
              </div>
              ...
              <div className="flex w-full flex-col justify-between gap-10 lg:flex-row lg:items-end">
                <div className="shrink-0">
                  ...
                  >
                    <div
                      className="absolute top-1/2 left-1/2 h-[calc(100%+16px)] w-[calc(100%+16px)] -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[#b59842] bg-[#b59842]/10 opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                ...
                <div className="relative max-w-[450px] overflow-hidden rounded-2xl border border-black/10 bg-black/5 p-8">
                  <div className="absolute top-0 left-0 h-[2px] w-[120px] bg-[#b59842]" />
                  <div className="flex gap-4">
                    <span className="font-serif text-4xl leading-none text-black opacity-30">
                      "
                    </span>
                    <p className="pt-1 text-base font-medium leading-relaxed italic text-black md:text-lg">
